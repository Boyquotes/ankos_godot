...
void fragment() {
	...
	unit = normalize(unit);
	// generate noise
	// multiple noise layers at diff freq to make cool shit
	float n = noise(unit * 5.0) * 0.5;
	n += noise(unit * 10.0) * 0.25;
	n += noise(unit * 20.0) * 0.125;
	n += noise(unit * 40.0) * 0.0625;
	// output colour
	// uses mix -- aka lerp -- which interpolates between A and B by C amount
	// need to use smoothstep to get clear sep between values
